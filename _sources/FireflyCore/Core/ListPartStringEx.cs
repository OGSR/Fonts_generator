...
/// </summary>
    public class ListPartStringEx<T> : ICollection<T>, IEquatable<ListPartStringEx<T>>, IComparable<ListPartStringEx<T>>, ICloneable
    {
        ...
        public ListPartStringEx(IList<T> Data)
        {
            if (Data is null)
        ...
        public ListPartStringEx(IList<T> Data, int HashCode)
        {
            if (Data is null)
                throw new ArgumentNullException();
            Internal = Data;
        ...
        /// <summary>已重载。考虑到效率原因，不会复制数据，而是直接引用数据，因此传入的数组不得改变</summary>
        public ListPartStringEx(IList<T> Data, int Offset, int Length)
        {
            if (Data is null)
                throw new ArgumentNullException();
            Internal = new ListPart<T>(Data, Offset, Length);
        ...
        /// <summary>已重载。考虑到效率原因，不会复制数据，而是直接引用数据，因此传入的数组不得改变</summary>
        public ListPartStringEx(IList<T> Data, int Offset, int Length, int HashCode)
        {
            if (Data is null)
                throw new ArgumentNullException();
            Internal = new ListPart<T>(Data, Offset, Length);
        ...
        public ListPartStringEx(ListPartStringEx<T> BaseString, int ExpandLength)
        {
            Internal = new ListPart<T>(BaseString.InternalList, BaseString.Offset, BaseString.Length + ExpandLength);

            InternalList = BaseString.InternalList;
            Offset = BaseString.Offset;
            Length = BaseString.Length + ExpandLength;

            int hash = BaseString.HashCode;
            foreach (var v in new ListPart<T>(BaseString.InternalList, BaseString.Offset + BaseString.Length, ExpandLength))
                hash = hash << 5 ^ v.GetHashCode() ^ hash >> 2 & 0x3FFFFFFF;
            HashCode = hash;
        }

        /// <summary>已重载。延长已有的串。考虑到效率原因，不会复制数据，而是直接引用数据，因此传入的数组不得改变</summary>
        public ListPartStringEx(ListPartStringEx<T> BaseString, int ExpandLength, int HashCode)
        {
            Internal = new ListPart<T>(BaseString.InternalList, BaseString.Offset, BaseString.Length + ExpandLength);
        ...
        public bool Equals(ListPartStringEx<T> other)
        {
        ...
        public override bool Equals(object obj)
        {
            ...
            if (obj is null)
                return false;
            ListPartStringEx<T> s = obj as ListPartStringEx<T>;
            if (s is null)
                return false;
            return Equals(s);
        }
        ...
        public int CompareTo(ListPartStringEx<T> other)
        {
            if (other is null)
                return int.MinValue;
        ...
        public ListPartStringEx<T> Reverse()
        {
            return Reverse(0, Internal.Count);
        }

        public ListPartStringEx<T> Reverse(int Index, int Count)
        {
            ...
            return new ListPartStringEx<T>(Result);
        }

        public static implicit operator StringEx<T>(ListPartStringEx<T> l)
        {
        ...
        public int IndexOf(ListPartStringEx<T> Value)
        {
            return IndexOf(Value, 0, Internal.Count);
        }

        public int IndexOf(ListPartStringEx<T> Value, int Index)
        {
            return IndexOf(Value, Index, Internal.Count - Index);
        }

        public int IndexOf(ListPartStringEx<T> Value, int Index, int Count)
        {
            if (Index < 0 || Index >= Internal.Count)
                throw new ArgumentOutOfRangeException();
            if (Index + Count > Internal.Count)
        ...
        public int LastIndexOf(ListPartStringEx<T> Value)
        {
            return LastIndexOf(Value, Internal.Count - 1, Internal.Count);
        }

        public int LastIndexOf(ListPartStringEx<T> Value, int Index)
        {
            return LastIndexOf(Value, Index, Index + 1);
        }

        public int LastIndexOf(ListPartStringEx<T> Value, int Index, int Count)
        {
            if (Index < 0 || Index >= Internal.Count)
                throw new ArgumentOutOfRangeException();
            if (Index >= Internal.Count)
        ...
        public static bool operator ==(ListPartStringEx<T> Left, ListPartStringEx<T> Right)
        {
        ...
        public ListPartStringEx<T> Substring(int Index, int Length)
        {
            return new ListPartStringEx<T>(new ListPart<T>(Internal, Index, Length));
        }

        public ListPartStringEx<T> Substring(int Index)
        {
            return new ListPartStringEx<T>(new ListPart<T>(Internal, Index, Internal.Count - Index));
        }
    }
}